...
def normalize_user_rating_matrix():
    ...
    index = 0
    user_rating_matrix__normalized = user_rating_matrix.getrow(index) / m.sqrt(lenght_vector.item(index))
    lenght_vector = np.delete(lenght_vector, index)
    for item in lenght_vector:
        index += 1
        matrix_row = user_rating_matrix.getrow(index)
        if item == 0:
            user_rating_matrix__normalized = vstack([user_rating_matrix__normalized, matrix_row])
        else:
            user_rating_matrix__normalized = vstack([user_rating_matrix__normalized, matrix_row / m.sqrt(item)])

    save_sparse_csc("../precomputedData/userRatingsMatrixNormalized.npz", user_rating_matrix__normalized.tocsc())
...
def create_user_rating_matrix_similarity():
    ...
    matrix_similarity_user = np.dot(user_rating_matrix.getrow(0), user_rating_matrix.T)
    for index in rows_array:
        tic = dt.now()
        if index != 0:
            new_row = np.dot(user_rating_matrix.getrow(index), user_rating_matrix.T)
            matrix_similarity_user = vstack([matrix_similarity_user, new_row])
        print("user {} computed in: {}".format(index, dt.now() - tic))

    save_sparse_csc("../precomputedData/userRatingSimilarity.npz", matrix_similarity_user.tocsc())
